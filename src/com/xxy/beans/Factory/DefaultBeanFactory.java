...
import com.xxy.beans.Factory.BeanFactory;
...
public class DefaultBeanFactory implements IBeanBuilder {
    ...
    public void packageScan(String packagePath) {
        new PackageScanner() {
            ...
            public void dealClass(Class<?> klass) {
                dealKlass(klass, beanFactory);
            }

        }.packageScanner(packagePath);
    }

    private void dealKlass(Class<?> klass, BeanFactory beanFactory) {
...
//       在形成代理时是否已经有了相应的拦截器链并不重要,但是在程序启动之前必须对拦截器类进行扫描
        try {
            ...
            beanFactory.setBeanDefinitionHolder(beanDefinitionHolder);
    ...
    public Object getObject(Class<?> klass) {
        return beanFactory.getBeanDefinitionHolder(klass.getName()).getObject();
    }

    @Override
    public <T> T getproxy(Class<?> klass) {
        try {
            return beanFactory.getBean(klass.getName());
        } catch (Exception e) {
            e.printStackTrace();
        }
